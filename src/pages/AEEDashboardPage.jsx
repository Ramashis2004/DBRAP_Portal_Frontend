...
import {
  fetchAEEDashboardApplicationSummary,
...
function AEEDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchAEEDashboardApplicationSummary(parsedSession.id)
      .then((response) => setSummaryCount(Number(response.data?.totalApplications || 0)))
      .catch((err) => {
        console.error("AEE dashboard summary failed:", err);
        setSummaryCount(0);
      })
  ...
  useEffect(() => {
    if (!session?.id || !showApplications) return;

    setIsLoadingApps(true);
    setError("");
    fetchAEEDashboardApplications(session.id)
      .then((response) => setApplications(Array.isArray(response.data) ? response.data : []))
      .catch((err) => {
        console.error("AEE dashboard applications failed:", err);
        setError(err.response?.data?.error || "Failed to load applications.");
      })
      .finally(() => setIsLoadingApps(false));
  }, [session?.id, showApplications]);
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    const matchesSearch =
      ...
      app.organisation_name?.toLowerCase().includes(query) ||
      app.block?.toLowerCase().includes(query) ||
  ...
  const renderDocumentLink = (app, documentType, label = "View File") => {
    ...
    return (
      <button
        type="button"
        className="se-dashboard-app-link"
  ...
  const mainContent = detailView ? (
      ...
      <div className="se-dashboard-app-section-grid">
        ...
        <SectionBox title="Site Visit Report">
          <Row
            ...
            value={detailView.site_visit_report ? (
              <button
                type="button"
                className="se-dashboard-app-link"
    ...
    <section className="se-dashboard-app-table">
      <div className="se-dashboard-app-table__top">
        <button type="button" className="se-dashboard-app-back" onClick={() => setShowApplications(false)}>
          <ArrowLeft size={16} />
          Back to Dashboard
        </button>
      </div>
      <div className="se-table-header">
        ...
        <div className="se-dashboard-app-table-controls">
          <select className="se-status-select" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
            ...
            <thead>
              <tr>
                {tableColumns.map((column) => (
                  <th key={column.label} style={{ width: `${column.width}px` }}>{column.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filteredApplications.length === 0 ? (
                ...
                <tr key={app.application_id} className={index % 2 === 0 ? "" : "is-alt"}>
                  ...
                  <td>
                    <span className="se-dashboard-app-pill" style={getActionStatusMeta(app)}>
    ...
    <section className="officer-dashboard-stats se-dashboard-stats">
      <button type="button" className="se-dashboard-app-card" onClick={() => setShowApplications(true)}>
        <div className="se-dashboard-app-card__icon"><FileText size={22} /></div>
        <div>
          <strong>{isLoadingSummary ? "..." : summaryCount}</strong>
        ...
        <aside className="officer-dashboard-sidebar">
          <div className="officer-dashboard-sidebar__brand">
            ...
            <div>
              ...
              <strong>AEE Workspace</strong>
            </div>
          </div>
          <nav className="officer-dashboard-nav">
            <div className="officer-dashboard-nav__group">
              <button type="button" className="officer-dashboard-nav__item is-active" onClick={() => {
          ...
          <header className="officer-dashboard-header">
            ...
            <div className="officer-dashboard-user">
              <div>
                ...
                <strong>{session?.loginId || session?.username}</strong>
              </div>
