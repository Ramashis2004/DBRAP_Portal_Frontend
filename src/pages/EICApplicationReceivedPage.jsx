...
import "./EICApplicationReceivedPage.css";
...
/**
 * Fetch applications.
 * blockCode = ""  → "All Circles" path (backend ignores block filter)
 * blockCode = "X" → specific block
 */
const loadApplications = async (officerId, blockCode) => {
  const res = await fetchEICApplicationReceivedApplications(
    officerId,
    blockCode,   // empty string triggers the All path on the backend
    ALL_STATUSES
  );
  return res.data || [];
};

const forwardToJE = (applicationId) =>
  updateOrganisationStatus(applicationId, "APPLICATION_FORWARDED_TO_JE");

const approveApplication = (applicationId) =>
  updateOrganisationStatus(applicationId, "APPLICATION_APPROVED");

const daysBetween = (from, to) => {
  if (!from) return 0;
  const start = new Date(from); start.setHours(0, 0, 0, 0);
  const end   = to ? new Date(to) : new Date(); end.setHours(0, 0, 0, 0);
...
const getApplicationStatusStyle = (status) => {
  switch (String(status).toUpperCase()) {
    ...
    case "APPLICATION_APPROVED": return { background: "#dcfce7", color: "#92400e" };
    ...
    case "PAYMENT_RECEIPT_REJECTED_FOR_CANCELLATION": return { background: "#fee2e2", color: "#b91c1c" };
...
const getActionTakenDate = (app) => {
  const status = String(app.application_status || "").toUpperCase();
  if (status === "APPLICATION_REJECTED")        return app.rejected_on ?? app.update_on ?? null;
...
function EICApplicationReceivedPage() {
  ...
  // Convenience: is "All Circles" selected?
  ...
  // ── Load all circles (EIC sees every circle) ──────────────────────────────
  ...
  const handleCircleChange = useCallback(async (e) => {
    ...
    // "ALL" — no cascade needed, downstream dropdowns stay empty/disabled
  ...
  const handleSubmit = useCallback(async () => {
    // "All Circles" requires only a circle selection (or ALL).
    // Normal path requires a block selection.
    ...
    try {
      // Pass empty string for "All Circles" → backend All path
  ...
  const handleOptionClick = (option) => {
    ...
    if (url === "/createuser"          || label === "create user")          navigate("/dashboard");
    if (url === "/applicationreceived" || label === "application received") { handleReset(); navigate("/eic-application-received"); }
  };
  ...
  // All Circles: only circle must be "ALL" (no block needed)
  // Normal:      a specific block must be selected
      ...
      <div className="ce-table-section">
        {!hasSubmitted ? (
          <div className="ce-prompt-box">
            ...
            <p>Choose "All Circles" to see every application, or drill down to a specific block.</p>
          </div>
        ) : isLoadingApps ? (
...
export default EICApplicationReceivedPage;
